-- Corrige reingreso en team_pick_*:
-- si el usuario ya tenía fila cancelada, al volver a unirse se reactiva en vez
-- de devolver éxito silencioso sin aparecer en el listado actual.
...
BEGIN
  ...
  -- Ya activo en el partido: no duplicar.
  IF EXISTS (
    SELECT 1
  ...
  -- Caso bug: si ya existía cancelado, reactivar y actualizar equipo/rol.
  UPDATE public.match_opportunity_participants p
   ...
   WHERE p.opportunity_id = p_opportunity_id
     AND p.user_id = auth.uid()
     AND p.status NOT IN ('pending', 'confirmed');
...
EXCEPTION
  WHEN unique_violation THEN
    -- Condición de carrera: otra transacción ya insertó/activó.
    RETURN jsonb_build_object('ok', true);
