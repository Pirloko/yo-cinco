-- Duelos rival aceptados (equipo vs equipo): visibles en listado y detalle para cualquier
-- jugador autenticado; unirse sigue restringido en join_match_opportunity (solo plantel).
  ...
  USING (
    ...
    OR (
      ...
      AND EXISTS (
        ...
        WHERE mo.id = rival_challenges.opportunity_id
          AND mo.type = 'rival'::public.match_type
          AND mo.status IN ('pending', 'confirmed', 'completed')
      )
    )
  );
...
COMMENT ON VIEW public.match_opportunities_masked IS
  'Lectura cliente: join_code en team_pick_private solo si aplica; rival aceptado visible para todos los autenticados; rival pendiente solo equipos del desafío.';
